...
void LAppModel::LoadAssets(const csmChar *dir, const csmChar *fileName)
{
    ...
    //LAppPal::PrintLog("[SGY]File Real path: %s", path.GetRawString());
...
void LAppModel::Update()
{
    ...
    if (_lipSync)
    {
        ...
        value = _wavFileHandler.GetRms() * 7.0f;
    ...
    //样例：sounds/vo_char_1001_00_01_hca.wav

    if (passVoice != "")
    {
        if (passVoice != "-1")
        {
            csmString path = csmString(passVoice.c_str());
            //path = _modelHomeDir + path;
            //JniBridgeC::PlaySoundFromJava(path.GetRawString());
            LAppPal::PrintLog("[SGY]Sound: %s", path.GetRawString());
            _wavFileHandler.Start(path);
        }
        else
        {
            JniBridgeC::PlaySoundFromJava("-1");
            _wavFileHandler.ReleasePcmData();
        }
        passVoice = "";
    }
...
CubismMotionQueueEntryHandle LAppModel::StartMotion(const csmChar *group, csmInt32 no, csmInt32 priority, ACubismMotion::FinishedMotionCallback onFinishedMotionHandler, const char *sgy_name)
{
    if (sgy_name != nullptr)
    {
        // Read No.
        for (no = 0; no < _modelSetting->GetMotionCount(group); no++)
        {
            if (strcmp(sgy_name, (_modelSetting->GetMotionFileName(group, no))) == 0)
            {
                break;
            }
        }
    ...
    //filename example:"mtn/motion_001.motion3.json"
    //LAppPal::PrintLog("[SGY]Show Motion: %s. ", name.GetRawString());
    //LAppPal::PrintLog("[SGY]Show Filename: %s. ", fileName.GetRawString());
...
bool IsHit_SGY(float pointX, float pointY)
{
    ...
    LAppPal::PrintLog("[SGY]picInfo:Px:%d,Py:%d,Mw:%d,Mh:%d",pointX,y,maxWidth,maxHeight);
    return (pointX >= (maxWidth*0.25f) && pointX <= (maxWidth*0.75f) && y <= (maxHeight*0.9f) && y >= (maxHeight*0.1f));
}


csmBool LAppModel::HitTest(const csmChar *hitAreaName, csmFloat32 x, csmFloat32 y ,float pointX, float pointY)
{
    // 透明時は当たり判定なし。
    //if (_opacity < 1)
    //{
    //    return false;
    //}
    return IsHit_SGY(pointX, pointY);
    ...
    //        LAppPal::PrintLog("[SGY]isHit? %s",(drawID == NULL)? "Null":"not Null");
...
void LAppModel::SetRandomExpression()
{
    ...
    for (map_ite = _expressions.Begin(); map_ite != _expressions.End(); map_ite++)
    {
        if (i == no)
        {
            ...
            LAppPal::PrintLog("[SGY]Show Expression: %s ", name.GetRawString());
            SetExpression(name.GetRawString());
            //SetExpression("mtn_ex_011.exp3.json");
            return;
        }
        i++;
    }
