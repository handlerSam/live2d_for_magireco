...
void LAppView::ChangeBackground(string imageName){
...
{
    if(passBackgroundName != nowBackgroundName){
        int width = LAppDelegate::GetInstance()->GetWindowWidth();
        int height = LAppDelegate::GetInstance()->GetWindowHeight();
        LAppPal::PrintLog("[SGY]background3, %s", (char*)(passBackgroundName.c_str()));
        LAppTextureManager *textureManager = LAppDelegate::GetInstance()->GetTextureManager();
        const string resourcesPath = ResourcesPath;

        LAppTextureManager::TextureInfo *backgroundTexture = textureManager->CreateTextureFromPngFile(resourcesPath + passBackgroundName);
        ...
        _renderSprite = new LAppSprite(x, y, width, height, 0, _programId);
        LAppPal::PrintLog("[SGY]background4, %s", (char*)(passBackgroundName.c_str()));
        nowBackgroundName = passBackgroundName;
    }
    _back->Render();
    //_gear->Render();
    //_power->Render();
    ...
    //{
    //    _changeModel = false;
    //    //ReadJsonFromFile("100650/100650.json");
...
void LAppView::OnTouchesEnded(float pointX, float pointY)
{
    ...
    live2DManager->OnDrag(0.0f, 0.0f);
    {
        ...
        live2DManager->OnTap(x, y,pointX,pointY);
